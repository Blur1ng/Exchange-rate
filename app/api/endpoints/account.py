...
async def verify_email(request: Request, user_url: str, db: AsyncSession = Depends(get_db), token: str = Depends(get_jwt_from_cookie)):
    try:
        user_id = await redis_client.get(f"{user_url}")
        if not user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Срок действия ссылки истек")
        user_id = int(user_id.decode())
        user: User = await GetData(db, User).from_id(user_id)
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Пользователь не найден")
        user.is_verified = True
        return templates.TemplateResponse("verify_email.html", {"request": request, "title": "VERIFIED EMAIL", "token": token})
